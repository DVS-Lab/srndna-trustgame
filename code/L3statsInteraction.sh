...
MAINOUTPUT=${maindir}/derivatives/fsl/L3_model-01_task-${task}_n${N}_flame1+2_interaction
mkdir -p $MAINOUTPUT
...
OUTPUT=${MAINOUTPUT}/L3_task-${task}_${REPLACEME}_cnum-${cnum_pad}_cname-${copename}
if [ -e ${OUTPUT}.gfeat/cope1.feat/cluster_mask_zstat1.nii.gz ]; then
	...
	ITEMPLATE=${maindir}/templates/L3_template_n${N}_${task}_interactionCovs.fsf
	OTEMPLATE=${MAINOUTPUT}/L3_task-${task}_${REPLACEME}_copenum-${copenum}_interactionCovs.fsf
